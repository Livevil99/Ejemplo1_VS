...
namespace POOU2D_Ejemplo1_
{
    class Alumno
    {
        ...
        private string matricula, nombreAlumno = string.Empty, apellidoPrimero, apellidoSegundo, domicilioAlumno, carrera ="Ing. Sistemas Computacionales";
        string curpAlumno, nombreInstitucion, contrasenaAlumno, grupoAlumno = "2D";
        ...
        public string CalcularEdadAlumno1() {            
            ...
            if (fechaNacimiento.Year < 1960)
            {
                valorDeRetorno = string.Format("La edad no ha sido asignada"+
                    "por favor asigna la edad al alumno/a: {0}{1}{2}", nombreAlumno, apellidoPrimero, apellidoSegundo);
            }
            else
            {
                edad = ((DateTime.Now - fechaNacimiento).Days) / 365;
                valorDeRetorno = string.Format("El alumno/a: {0}{1}{2}\n" +
                    "Matricula: {3}\n" +
                    "Edad: {4}\n" +
                    "Esta inscrito en la carrera de: {5}\n" +
                    "Grupo/seccion: {6}\n", nombreAlumno,apellidoPrimero,apellidoSegundo,matricula,edad,carrera,grupoAlumno);
            }   
